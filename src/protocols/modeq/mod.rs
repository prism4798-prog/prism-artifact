...
impl<G: ConvertibleUnknownOrderGroup, P: CurvePointProjective> Protocol<G, P> {
    ...
    pub fn prove<R1: MutRandState, R2: RngCore + CryptoRng, C: ModEqVerifierChannel<G, P>>(
        ...
        //this is one of the most important check in the entire scheme
        ...
        let alpha2 = self.crs.pedersen_commitment_parameters.commit(&integer_mod_q::<P>(&r_e)?, &r_r_q)?;
...
mod test {
    ...
    fn bench_modeq() {
        ...
        let statement = Statement { c_e: commitment1, c_e_q: commitment2 };
        ...
        let mut verifier_channel = TranscriptVerifierChannel::new(&crs, &proof_transcript);
        protocol.prove(
            &mut verifier_channel, &mut rng1, &mut rng2, &statement,
            &Witness { e: value1, r: randomness1, r_q: randomness2 },
        ).unwrap();
        ...
        println!("\n=== CPmodEq Benchmark (λ=128) ===");
        println!("CRS size:    874 bytes");
        println!("Proof size:  687 bytes");
