...
mod test {
    ...
    use crate::{
        ...
        parameters::Parameters,
        protocols::{
            hash_to_prime::snark_hash::{Protocol as HPProtocol, test::TestParameters},
    ...
    fn bench_root() {
        ...
        let mut verifier_channel = TranscriptVerifierChannel::new(&crs, &proof_transcript);
        protocol.prove(
            &mut verifier_channel, &mut rng1, &statement,
            &Witness { e: value, r: randomness, w },
        ).unwrap();
        ...
        // Proof size — count bytes in each integer using hex representation
        let msg3_size =
            proof.message3.s_e.significant_digits::<u8>()
            + proof.message3.s_r.significant_digits::<u8>()
            + proof.message3.s_r_2.significant_digits::<u8>()
            + proof.message3.s_r_3.significant_digits::<u8>()
            + proof.message3.s_beta.significant_digits::<u8>()
            + proof.message3.s_delta.significant_digits::<u8>();

            println!("\n=== CPRoot Benchmark ===");
            println!("CRS size:    ~778 bytes");
            println!("  Parameters:    10 bytes");
            println!("  g, h:          512 bytes (2 × RSA2048)");
            println!("  N (implicit):  256 bytes (RSA2048 modulus)");
            println!("Proof size:  ~{} bytes", 512 + 1024 + msg3_size);
            println!("  Message1:      512 bytes (c_w, c_r: 2 × RSA2048)");
            println!("  Message2:      1024 bytes (alpha1..4: 4 × RSA2048)");
            println!("  Message3:      ~{} bytes (6 integers)", msg3_size);
