# PRISM: Privacy-Preserving Email via CP-SNARK Set Membership Proofs
...
**JSON (stdout):** ciphertext, nonce, commitment, ephemeral_pub_{x,y}, proof_b64, recipient_ciphertext
...
```
Alice (Nodemailer/sendmail_zkp.js)
  │
  ├── Calls ./prover → proof + encrypted From/To
  │
  └── Nodemailer sends via SMTP port 587
        From: <ciphertext>@senderdomain.org
        To: <ciphertext>@receiverdomain.org
...
├── target/release/{setup, prover, verifier, decrypt, auth_setup, auth_prover, auth_verifier, token_issue}
├── zkp-data/{crs.bin, accumulator.bin, vk.bin, witnesses/, auth_*.bin}
└── users.csv

/home/ubuntu/mailtest/
└── sendmail_zkp.js   (Nodemailer, calls ./prover, sends via port 587)
```
...
/usr/local/bin/zkp_filter.sh               # Postfix content filter
...
```bash
ssh <sender-mta>
cd /home/ubuntu/mailtest && node sendmail_zkp.js
```
...
**Expected output:**
```
=== Auth Circuit (LegoGroth16) ===
Constraints: 7266
Setup:       230 ms
Prove:       170 ms (median of 3: 170, 170, 171)
Verify:      7 ms
PK size:     2031 KB
VK size:     632 bytes
Proof size:  336 bytes
```
...
**Why dummy constraints are representative:** Groth16 proving time is dominated by multi-scalar multiplication (MSM), which scales with the number of constraints, not with what the constraints compute. Field multiplications have the same R1CS cost as Poseidon sponge or elliptic curve scalar multiplication constraints. The dummy approach produces the same constraint count (12,295) and therefore the same MSM workload. The 5,029 additional constraints were calculated from measured per-operation costs in CP_IdEnc:

```
Step 2 — ECDH:     3,949 constraints (variable-base scalar mul on JubJub)
Step 3 — KDF:        540 constraints (Poseidon sponge)
Step 5 — PRF:        540 constraints (Poseidon sponge)
                   ─────
Total additional:  5,029 constraints
```

These exact operations already exist in the CP_IdEnc circuit (`snark_hash.rs`). Porting them into `AuthCircuit` as real constraints is straightforward but was not completed before submission. The dummy constraint benchmark is a valid proxy for the proving time reported in the paper.
...
Constraints: 12295
Setup:       507 ms
Prove:       291 ms (median of 3: 289, 291, 291)
Verify:      2994 us
PK size:     6093984 bytes (5951 KB)
VK size:     632 bytes
...
The parallel scaling data for all three circuits is measured by setting `RAYON_NUM_THREADS` before each test run:
...
# CP_IdEnc (LegoGroth16, 6,456 constraints) — via prover binary
for t in 1 2 4 6 8 10 12 14 16; do
    echo -n "Threads=$t: "
...
All three plateau at 12-16 threads with 6-7× speedup. The receiver circuit benefits slightly more from parallelism (7×) due to the larger MSM workload.
...
|--------|------|-------------|
| Sender MTA | Authenticated user sent email | Recipient identity (encrypted To:) |
| Receiver MTA | Valid ZK proof from accumulator member | Sender identity (encrypted From:), Recipient identity (encrypted To:) |
...
| Postfix content filter (membership verify) | `zkp_filter.sh` | Integrated with Postfix |
| ECDH decrypt + common mailbox | `decrypt` | Full implementation |
| End-to-end email pipeline | `sendmail_zkp.js` | Sender → relay → verify → deliver → decrypt |
...
The numbers in the paper are produced by the test functions, not the integrated binaries. The key invariants:

1. **Constraint counts are exact** (7,266 and 12,295) — measured from the actual R1CS constraint systems.
2. **Proving times are MSM-dominated** — dummy constraints produce the same MSM workload as real constraints. Validated by comparing CP_IdEnc's real ECDH (3,949 constraints, measured) with proportional scaling.
...
```bash
./target/release/bench_accumulator
# Table 1: Core ops (10-10K users) — Build, Build-T, Wit, Wit-T, WitUpd, Add, Del-T, Verify
...
```bash
# Sender
echo "START: $(date +%s.%N)" && cd /home/ubuntu/mailtest && node sendmail_zkp.js && echo "SENT: $(date +%s.%N)"

# Receiver
echo "WAITING: $(date +%s.%N)"
while [ -z "$(ls /var/mail/zkp-common/ 2>/dev/null)" ]; do sleep 0.1; done
echo "ARRIVED: $(date +%s.%N)"
sudo su - bob -c "/usr/local/bin/decrypt ..." && echo "DONE: $(date +%s.%N)"
# Steady state: ~902ms
```
